...
- **Deferred follow-ups:** off-thread decode-ahead worker; audio-master A/V sync. Both are documented phase-2 work, additive rather than rewrites — a worker slots between the clock and `RenderFrame` / the audio ring without changing the seams.
