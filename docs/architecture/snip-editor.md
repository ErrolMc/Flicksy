...
| Control | Notes |
| --- | --- |
| [ImageEditToolsView](../../Flicksy.PostSnip/Controls/ImageEditToolsView.xaml.cs) | Centered toolbar. Click on already-active Pen/Shapes/Text toggles its settings popup. Opening the Text popup with a text selected syncs the popup to it and begins a `TextStyleCommand` snapshot; closing pushes the diff. The Shape popup does the same for a selected shape (`SyncFromShapeItem` + `ShapeStyleCommand`), and the Pen popup for a selected stroke (`SyncFromPenStrokeItem` + `PenStyleCommand`). A selected shape/stroke survives the Select→Shapes/Pen tool switch via `PostSnipViewModel`'s `keepShapeSelection`/`keepPenSelection` (mirrors of `keepTextSelection`). |
...
| Restyle a selected text/shape/pen stroke after placement | item setters (`SetFill`/`SetOutline`/`SetStyle`/...) in [TextItem](../../Flicksy.Drawing/Source/TextItem.cs)/[ShapeItem](../../Flicksy.Drawing/Source/ShapeItem.cs)/[PenStrokeItem](../../Flicksy.Drawing/Source/PenStrokeItem.cs), the `Begin/EndXxxStyleEdit` pair in [DrawingViewModel](../../Flicksy.Drawing/ViewModels/DrawingViewModel.cs), the `OnSelectedXxxStyleChanged` DP callbacks in [DrawingView.DependencyProperties.cs](../../Flicksy.Drawing/Controls/DrawingView/DrawingView.DependencyProperties.cs), `SyncFromXxxItem` in the settings VM, and the popup `Opened`/`Closed` handlers in [ImageEditToolsView.xaml.cs](../../Flicksy.PostSnip/Controls/ImageEditToolsView.xaml.cs). |
