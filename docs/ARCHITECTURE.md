...
| [ICompositor.cs](../Flicksy.VideoEditor/Composition/ICompositor.cs) | Backend-neutral **video** seam. `RenderFrame(project, frame, target)` paints into a caller-owned `WriteableBitmap` (sized to project resolution; reused across frames, unfrozen → same-thread presentation). Sync, single-call-in-flight; owns its own decoder cache so callers see only the painted bitmap. `IDisposable`. Audio mixing is the separate [IAudioMixer](../Flicksy.VideoEditor/Composition/IAudioMixer.cs) seam (ADR 0005). |
| [SkiaCompositor.cs](../Flicksy.VideoEditor/Composition/SkiaCompositor.cs) | SkiaSharp/CPU `ICompositor` impl — **video only**. Paints into the caller-supplied `WriteableBitmap` (validates its size == project resolution, then `Lock`/`InstallPixels`/paint/`AddDirtyRect`/`Unlock`) — no per-frame allocation, no `Freeze`; the back buffer is wrapped via `SKBitmap.InstallPixels(...)` so paint lands directly in WPF-bindable memory with no intermediate Skia surface. Layers paint in `CompositionPlanner.PlanFrame` order: `MediaClip` (`Streams ∈ {Video, Both}`) decodes via the cached `IMediaDecoder`, pins the rented frame buffer, draws via `SKImage.FromPixels` (`SKAlphaType.Opaque`) under the layer matrix; `GraphicsClip` renders through `DrawingVisual` + `RenderTargetBitmap` then bounces into Skia (constrains the call to a UI-Dispatcher thread for projects with graphics). Matrix: `M = T_clipCenter * R * S * T_-sourceCenter` where `clipCenter = projectCenter + Transform.Position`; `CropRect` (when set) shifts the source center to crop-center and applies a `canvas.ClipRect` in source space. Holds a [MediaDecoderCache](../Flicksy.VideoEditor/Composition/MediaDecoderCache.cs) (video cursors), disposed in `Dispose`. |
| [MediaDecoderCache.cs](../Flicksy.VideoEditor/Composition/MediaDecoderCache.cs) | `Clip.Id`-keyed `Dictionary<Guid, IMediaDecoder>` with lazy `GetOrCreate(clip, sampleRate)` (null source / open failure → null, clip renders nothing) and `Dispose` (disposes all). One instance per seam — `SkiaCompositor` and `AudioMixer` hold separate caches so a `Streams=Both` clip gets two decoders (independent video + audio cursors; FFmpeg seeks the container, so a shared `MediaFile` would thrash). Single-threaded by construction, no internal locking. |
